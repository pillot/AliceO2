...
//  memset(&buffer, 0, buffer.size());
    ...
    auto channelHandler = [&ndigits, &digits](DsElecId dsId, uint8_t channel, o2::mch::raw::SampaCluster sc) {
...
//      std::cout << "This part contained " << sc.nofSamples() << " samples. Of total ADC " << digitadc << std::endl;
        //Attention, mieux vaut charger la segmentation de mapping factory et le faire en dehors de toute boucle
  ...
  size_t nrdhs{0};
    ...
    auto rdhHandler = [&](const RDH& rdh) -> std::optional<RDH> {
      ...
      auto solar = cruLink2solar(o2::mch::raw::CruLinkId(cruId, linkId));
        ...
        r.feeId = solar.value();
            std::cout << "INFO:\nCRUID " << cruId << " LINKID " << int(linkId) << std::endl;
      return r;
    };
