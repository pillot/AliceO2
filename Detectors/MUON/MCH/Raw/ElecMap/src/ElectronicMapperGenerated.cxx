...
extern void fillElec2DetCH8L(std::map<uint16_t, uint32_t>& e2d);
...
extern void fillSolar2CruLinkCH8L(std::map<uint16_t, uint32_t>& s2c);
  ...
  createCruLink2SolarMapper<ElectronicMapperGenerated>()
{
  ...
  for (auto p : solarId2CruLinkId) {
    cruLinkId2SolarId[p.second] = p.first;
      std::cout << "[CreateCruLink2SolarMapper] cruLinkId2SolarId[" << p.second << "] = " << p.first << std::endl;
  }
